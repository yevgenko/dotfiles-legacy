...
def extend_console(name, care = true, required = true)
  if care
    ...
    $console_extensions << "#{ANSI[:LCYAN]}#{name}#{ANSI[:RESET]}"
  end
rescue LoadError
...
extend_console 'shopify_api' do
  ...
  def shopify
    return @shopify unless @shopify.nil?
    @shopify = ShopifyAPI::Irb.new
  end
end

# Show results of all extension-loading
puts "#{ANSI[:LCYAN]}~> Console extensions:#{ANSI[:RESET]} #{$console_extensions.join(' ')}#{ANSI[:RESET]}"
